...
if(!empty($_POST)){
	...
	if(empty($error)){
		...
		header('location: http://' . $_SERVER['HTTP_HOST'] . '/Twitter_bbs/join/check.php');
		exit();
	}
	//書き直し
...
<form action="" method="post" enctype="multipart/form-data">
	<dl>
		...
		<dd>
			...
			<?php if(isset($error['name'])): ?>
				...
				<p class="errror">* ニックネームを入力してください</p>
				<?php endif; ?>
			<?php endif; ?>
		</dd>
		...
		<dt>パスワード<span class="requeired">必須</span></dt>
		<dd>
			<?php if(!isset($_POST['password'])): ?>
				<input type="password" name="password" size="35" maxlength="255" />
			<?php endif; ?>
			<?php if(isset($_POST['password'])): ?>
				<input type="password" name="password" size="35" maxlength="255" value="<?php echo htmlspecialchars($_POST['password'] , ENT_QUOTES , 'UTF-8');?>" />
			<?php endif; ?>
	...
	<div><input type="submit" class="btn btn-default" value="入力内容を確認する" /></div>
</form>
